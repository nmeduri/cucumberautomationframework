...
public class AEM_Link_Existing_Card_Page extends PageObject {
	...
	public void displayErrorImageAEM() throws Exception {
		testContext.getPageObjectManager().getPageObject(PageObject.getDriver()).scrollDown(ExpectedConditions.presenceOfElementLocated(By.xpath(testContext.getPageObjectManager().getAEMLinkExistingCardLocatorPage().get_Error_Image())), 5);
		$display($(Loc.XPATH, testContext.getPageObjectManager().getAEMLinkExistingCardLocatorPage().get_Error_Image()));
	}
	
	/** enter Data to Error Image*/
	public void enterDataErrorImage() throws Exception {
		$clearData(ExpectedConditions.visibilityOfElementLocated(By.xpath(testContext.getPageObjectManager().getAEMLinkExistingCardLocatorPage().get_Error_Image())), 5);
		$enterData(ExpectedConditions.visibilityOfElementLocated(By.xpath(testContext.getPageObjectManager().getAEMLinkExistingCardLocatorPage().get_Error_Image())), 5, FileReaderManager.getInstance().getAEMDataReader().get_Active_Card_Error_Image_data());
	}
