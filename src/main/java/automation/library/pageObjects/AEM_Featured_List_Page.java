...
public class AEM_Featured_List_Page extends PageObject {
	...
	public void enterFLTitle() throws Exception {
		...
		$enterData($(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getFLTitle()), FileReaderManager.getInstance().getAEMDataReader().get_FL_Title());
	}

	/** verify authoring Guide lines For FL Title */
	public void authoringGuidelinesForFLTitle() throws Exception {
		$click(ExpectedConditions.elementToBeClickable($By(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getToolTipOfFLTitle())), 15);
		Thread.sleep(3000);
		//$display(ExpectedConditions.visibilityOfElementLocated($By(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getToolTipMessageOfFLTitle())),40);
		$displayFindElement(By.xpath(testContext.getPageObjectManager().getAEMFeatureListPageLocator().getToolTipMessageOfFLTitle()));
		//Thread.sleep(2000);
		//$display($(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getToolTipMessageOfFLTitle()));
		String mandatoryfield=$getText($(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getNotMandatoryConfiguration()));
	...
	public void verifyFeaturedProductListTitle() throws Exception {
		testContext.getPageObjectManager().getPageObject(PageObject.getDriver()).switchFrameByString("ContentFrame");
		$display(ExpectedConditions.visibilityOfElementLocated($By(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getTitleOfFeaturedProductList())),40);
		Assert.assertEquals($getText($(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getTitleOfFeaturedProductList())), FileReaderManager.getInstance().getAEMDataReader().get_FL_Title());
		PageObject.getDriver().switchTo().parentFrame();
	}
	...
	public void navigateTo_Featured_Product_List_Author_Page_Fr() throws Exception {
		driver.navigate().to(FileReaderManager.getInstance().getAEMDataReader().get_Featured_Product_List_AEM_Fr_Locale_Url());
	}
	/** Redirection label on featured list component */
	public void configureRedirectionLabel() throws Exception {
		$click(ExpectedConditions.elementToBeClickable($By(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getRedirectionLabel())), 15);
		//((JavascriptExecutor) PageObject.getDriver()).executeScript("arguments[0].click();", $findElement(By.xpath(testContext.getPageObjectManager().getAEMFeatureListPageLocator().getRedirectionLabel())));
		Thread.sleep(1000);
		$click(ExpectedConditions.elementToBeClickable($By(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getRedirectionLabelThisWindow())), 15);
		//((JavascriptExecutor) PageObject.getDriver()).executeScript("arguments[0].click();", $findElement(By.xpath(testContext.getPageObjectManager().getAEMFeatureListPageLocator().getRedirectionLabelThisWindow())));
	}
	...
	public void clickOnAEMProductCard() throws Exception {
		...
		$display(ExpectedConditions.visibilityOfElementLocated($By(Loc.XPATH, testContext.getPageObjectManager().getPDPPageLocator().get_Product_Title())),40);
	}
	/** enable of product badges on featured list component */
	public void enableOfProductBadges() throws Exception {
		$click(ExpectedConditions.elementToBeClickable($By(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getEnableBadges())), 20);
		$click(ExpectedConditions.elementToBeClickable($By(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getEnableBadges())), 20);
	}
	/** verify - Enabled Product Badges */
	public void verifyEnableProductBadges() throws Exception {
		testContext.getPageObjectManager().getPageObject(PageObject.getDriver()).switchFrameByString("ContentFrame");
		$listDisplay(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getEnableBadgesInPreview());	
		PageObject.getDriver().switchTo().parentFrame();
	}
	/** enable of product save story on featured list component */
	public void enableOfProductSaveStory() throws Exception {
		$click(ExpectedConditions.elementToBeClickable($By(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getEnableSaveStory())), 20);
		$click(ExpectedConditions.elementToBeClickable($By(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getEnableSaveStory())), 20);
	}
	/** verify - Enabled Product Save Story */
	public void verifyEnableProductSaveStory() throws Exception {
		testContext.getPageObjectManager().getPageObject(PageObject.getDriver()).switchFrameByString("ContentFrame");
		$display(ExpectedConditions.visibilityOfElementLocated($By(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getErrorAlertForMandatoryFieldAEM())),40);	
		PageObject.getDriver().switchTo().parentFrame();
	}
	/** Disable Product Description on featured list component */
	public void disableProductDescription() throws Exception {
		$click(ExpectedConditions.elementToBeClickable($By(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getDisableProductDescription())), 20);
		$click(ExpectedConditions.elementToBeClickable($By(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getDisableProductDescription())), 20);
	}
	...
	public void mandatorySectionsAreAuthored() throws Exception {
		$enterData($(Loc.XPATH, testContext.getPageObjectManager().getAEMFeatureListPageLocator().getProductCodeAEM()), FileReaderManager.getInstance().getAEMDataReader().get_Product_Code_AEM());
	}
	/** verify - Product Code */
