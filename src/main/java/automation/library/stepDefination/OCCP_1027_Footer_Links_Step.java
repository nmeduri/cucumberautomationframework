...
public class OCCP_1027_Footer_Links_Step extends BaseClass {
	...
	public void update_customer_service_destination_url() throws Exception {
		testContext.getPageObjectManager().getAEMFooterPage(PageObject.getDriver()).enterCustomerServerDestinationUrlDetails();
	}

	@Then("the customer service sub url changes are reflect on site")
	public void the_customer_service_sub_url_changes_are_reflect_on_site() throws Exception {
		testContext.getPageObjectManager().getAEMFooterPage(PageObject.getDriver()).verifyCustomerServiceUrlChangesReflectOnSite();
	}
	
	@And("reverse the customer service url changes")
	public void reverse_the_customer_service_url_changes() throws Exception {
		testContext.getPageObjectManager().getAEMFooterPage(PageObject.getDriver()).reverseChangesCustomerServerSubSectionUrlDetails();
	}
	
	@And("update service and solutions sub url details")
	public void update_service_and_solution_sub_url_details() throws Exception {
		testContext.getPageObjectManager().getAEMFooterPage(PageObject.getDriver()).enterServiceAndSolutionsSubUrlTitleDetails();
	}
	
	@Then("the service and solution sub url changes are reflect on site")
	public void the_service_and_solution_sub_url_changes_are_reflect_on_site() throws Exception {
		testContext.getPageObjectManager().getAEMFooterPage(PageObject.getDriver()).verifyServiceAndSolutionURlChangesReflectOnSite();
	}
	
	@And("reverse the service and solutions url changes")
	public void reverse_the_services_and_solutions_url_changes() throws Exception {
